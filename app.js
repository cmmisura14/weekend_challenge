...
function makeButton(object) {
	...
	newButton.onclick = function(){ 
		...
		amazingAttack.innerHTML = (object.playerClass ? 'with a Dexterity of ' : ' with a Strength of ') + object.attack(object.name);
	};
	return newButton;
}
...
function newEntry(){
	...
	if(document.querySelector('input[name = "class"]:checked')) {
			...
			newCreature = new Monster(document.getElementById('name').value.toUpperCase(), document.getElementById('strength').value, document.getElementById('dexterity').value, document.getElementById('constitution').value, document.getElementById('wisdom').value, document.getElementById('intelligence').value, document.getElementById('charisma').value);
		}
